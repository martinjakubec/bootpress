...
if ( ! function_exists( 'bootpress_setup' ) ) :
	...
	function bootpress_setup() {
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'bootpress_custom_background_args',
				array(
...
function bootpress_scripts() {
	wp_enqueue_style( 'bootpress-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'bootpress-style', 'rtl', 'replace' );

	wp_enqueue_script( 'bootpress-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
